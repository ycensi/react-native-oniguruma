# react-native-onigurama

React Native bridge package to Oniguruma regular expressions library (based on the archived [node-onigurama](https://github.com/atom/node-oniguruma/))
...
```js
import {OnigScanner, OnigRegExp} from 'react-native-onigurama';
```
...
```js
const scanner = new OnigScanner(['c', 'a(b)?'])
